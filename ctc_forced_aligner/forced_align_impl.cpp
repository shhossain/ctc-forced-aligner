...
void forced_align_impl(
    ...
    for (int batchIndex = 0; batchIndex < B; ++batchIndex) {
        const auto T = input_lengths_data(batchIndex);
        const auto L = target_lengths_data(batchIndex);
        const auto S = 2 * L + 1;
    ...
    for (auto t = 1; t < T; t++) {
      ...
      for (auto i = startloop; i < end; i++) {
        ...
        // In CTC, the optimal path may optionally choose to skip a blank label.
        ...
        // (i != 1) just ensures we don't access targets[i - 2] if i < 2
        if (i % 2 != 0 && i != 1 &&
    ...
    for (auto t = 0; t < T; ++t) {
      scores_data(batchIndex, t) = logProbs_data(batchIndex, t, paths_data(batchIndex, t));
    }
  }
}
...
std::tuple<py::array_t<int64_t>, py::array_t<float>> compute(
    ...
    if (input_lengths.ndim() != 1) throw std::runtime_error("input_lengths must be a 1-D array.");
    if (target_lengths.ndim() != 1) throw std::runtime_error("target_lengths must be a 1-D array.");
